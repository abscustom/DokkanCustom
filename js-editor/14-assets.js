...
async function fetchImageBlob(targetUrl) {
    ...
    // 1. Direct fetch for local or central repository URLs
    if (targetUrl.startsWith('data:') || targetUrl.startsWith('blob:') || targetUrl.startsWith('/') || targetUrl.startsWith('./') || targetUrl.startsWith('assets/') || targetUrl.includes('abscustom.github.io') || targetUrl.includes('localhost')) {
        try {
            let directRes = await fetch(targetUrl);
            if (directRes.ok) {
                let blob = await directRes.blob();
...
function generateCandidateCutinUrls(baseIdNum, root, seedIds = [], pastedIdNum = null, thumbIds = []) {
    ...
    const addPatternsForId = (id) => {
        candidates.add(`https://dokkaninfo.com/assets/japan/character/card/${id}/card_${id}_cutin.png`);
...
async function processAssets(textToSearch, htmlSource = "", options = {}) {
    ...
    const urlMatch = textToSearch.match(/dokkaninfo\.com\/cards\/(\d{7})/) || textToSearch.match(/\b(\d{7})\b/);
    if (!urlMatch) return false;
    const pastedId = parseInt(urlMatch[1], 10);
    ...
    const hdArtCandidateUrls = [
        ...
        `${root}/card/${baseIdNum}/${baseIdNum}.png`
    ];
    const hdArtUrl = hdArtCandidateUrls[0];
    const files = [
...
window.fetchAssetsOnly = async function() {
    ...
    if (!urlInput || !urlInput.value.trim()) {
        alert("Please paste a DokkanInfo URL or 7-digit Card ID in the box above first!");
        return;
    }
    ...
    try {
        ...
        let pageHtml = "";
        if (rawVal.includes('dokkaninfo.com/cards/')) {
            pageHtml = await fetchPageHtmlViaMirror(rawVal);
        } else if (/^\d{7}$/.test(rawVal)) {
            pageHtml = await fetchPageHtmlViaMirror(`https://dokkaninfo.com/cards/${rawVal}`);
        }
        await processAssets(rawVal, pageHtml, { showSAIconPopup: true });
